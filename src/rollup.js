...
MagicString.prototype.overwrite = function overwrite (start, end, content, options) {
	...
	while (start < 0) { start += this$1$1.original.length; }
	while (end < 0) { end += this$1$1.original.length; }
...
MagicString.prototype._split = function _split (index) {
	...
	while (chunk) {
		if (chunk.contains(index)) { return this$1$1._splitChunk(chunk, index); }
...
MagicString.prototype.trimStartAborted = function trimStartAborted (charType) {
	...
	do {
		...
		if (chunk.end !== end) {
			// special case...
			if (chunk === this$1$1.lastChunk) { this$1$1.lastChunk = chunk.next; }
...
pp$1.parseClass = function(node, isStatement) {
  ...
  while (!this.eat(types.braceR)) {
    ...
    if (member && member.type === "MethodDefinition" && member.kind === "constructor") {
      if (hadConstructor) { this$1$1.raise(member.start, "Duplicate constructor in the same class"); }
      hadConstructor = true;
    }
  }
