...
import path, { relative, dirname, basename, resolve, sep, extname } from 'path';
import { writeFile, readdirSync, mkdirSync, readFileSync, statSync, watch, lstatSync, realpathSync } from 'fs';
import crypto from 'crypto';
import module$1 from 'module';
import { EventEmitter } from 'events';
import { rollup as fakeRollup } from "../../../../../../../_snowpack/pkg/rollup.js";
/*
	Rollup.js v0.65.0
...
Mappings.prototype.addUneditedChunk = function addUneditedChunk(sourceIndex, chunk, original, loc, sourcemapLocations) {
  ...
  while (originalCharIndex < chunk.end) {
    if (this$1$1.hires || first || sourcemapLocations[originalCharIndex]) {
      this$1$1.rawSegments.push([this$1$1.generatedCodeColumn, sourceIndex, loc.line, loc.column]);
    }

    if (original[originalCharIndex] === '\n') {
      loc.line += 1;
      loc.column = 0;
      this$1$1.generatedCodeLine += 1;
      this$1$1.raw[this$1$1.generatedCodeLine] = this$1$1.rawSegments = [];
      this$1$1.generatedCodeColumn = 0;
    } else {
      loc.column += 1;
      this$1$1.generatedCodeColumn += 1;
    }
...
MagicString.prototype.indent = function indent(indentStr, options) {
  ...
  while (chunk) {
    ...
    if (chunk.edited) {
      ...
      while (charIndex < end) {
        if (!isExcluded[charIndex]) {
          var char = this$1$1.original[charIndex];
          ...
          if (char === '\n') {
            ...
            if (charIndex === chunk.start) {
              ...
              this$1$1._splitChunk(chunk, charIndex);
...
MagicString.prototype.overwrite = function overwrite(start, end, content, options) {
  ...
  while (start < 0) {
    start += this$1$1.original.length;
  }

  while (end < 0) {
    end += this$1$1.original.length;
  }
...
MagicString.prototype.remove = function remove(start, end) {
  ...
  while (chunk) {
    ...
    chunk.outro = '';
    chunk.edit('');
    chunk = end > chunk.end ? this$1$1.byStart[chunk.end] : null;
  }
...
MagicString.prototype._split = function _split(index) {
  ...
  while (chunk) {
    if (chunk.contains(index)) {
      return this$1$1._splitChunk(chunk, index);
    }

    chunk = searchForward ? this$1$1.byStart[chunk.end] : this$1$1.byEnd[chunk.start];
  }
};
...
MagicString.prototype.trimEndAborted = function trimEndAborted(charType) {
  ...
  do {
    ...
    if (chunk.end !== end) {
      if (this$1$1.lastChunk === chunk) {
        this$1$1.lastChunk = chunk.next;
      }

      this$1$1.byEnd[chunk.end] = chunk;
      this$1$1.byStart[chunk.next.start] = chunk.next;
      this$1$1.byEnd[chunk.next.end] = chunk.next;
    }
...
Bundle.prototype.trimEnd = function trimEnd(charType) {
  ...
  do {
    ...
    if (!source) {
      this$1$1.intro = this$1$1.intro.replace(rx, '');
      break;
    }
...
var CallOptions = function () {
  function CallOptions(_a) {
    var _b = _a === void 0 ? {} : _a,
        ...
        _d = _b.args,
        args = _d === void 0 ? [] : _d,
        _e = _b.callIdentifier,
        callIdentifier = _e === void 0 ? undefined : _e;
...
function assembleMemberDescriptions(memberDescriptions, inheritedDescriptions) {
  if (inheritedDescriptions === void 0) {
...
var UNKNOWN_EXPRESSION = {
  included: true,
  getLiteralValueAtPath: function () {
    return UNKNOWN_VALUE;
  },
  getReturnExpressionWhenCalledAtPath: function () {
...
var UnknownArrayExpression = function () {
...
var UnknownObjectExpression = function () {
...
function hasMemberEffectWhenCalled(members, memberName, parentIncluded, callOptions, options) {
  ...
  for (var _i = 0, _a = members[memberName].callsArgs; _i < _a.length; _i++) {
    var argIndex = _a[_i];
    if (callOptions.args[argIndex] && callOptions.args[argIndex].hasEffectsWhenCalledAtPath(EMPTY_PATH, CallOptions.create({
      withNew: false,
      args: [],
      callIdentifier: {} // make sure the caller is unique to avoid this check being ignored

    }), options.getHasEffectsWhenCalledOptions())) return true;
  }
...
var Variable = function () {
  ...
  Variable.prototype.getName = function (reset) {
    if (reset && this.safeName && this.safeName !== this.name && this.safeName[this.name.length] === '$' && this.safeName[this.name.length + 1] === '$') {
...
var LocalVariable = function (_super) {
  ...
  LocalVariable.prototype.getLiteralValueAtPath = function (path$$1, recursionTracker, origin) {
    if (this.isReassigned || !this.init || path$$1.length > MAX_PATH_DEPTH || recursionTracker.isTracked(this.init, path$$1)) {
      return UNKNOWN_VALUE;
    }
  ...
  LocalVariable.prototype.hasEffectsWhenAccessedAtPath = function (path$$1, options) {
    ...
    return this.isReassigned || path$$1.length > MAX_PATH_DEPTH || this.init && !options.hasNodeBeenAccessedAtPath(path$$1, this.init) && this.init.hasEffectsWhenAccessedAtPath(path$$1, options.addAccessedNodeAtPath(path$$1, this.init));
  };

  LocalVariable.prototype.hasEffectsWhenAssignedAtPath = function (path$$1, options) {
    if (this.included || path$$1.length > MAX_PATH_DEPTH) return true;
    if (path$$1.length === 0) return false;
    return this.isReassigned || this.init && !options.hasNodeBeenAssignedAtPath(path$$1, this.init) && this.init.hasEffectsWhenAssignedAtPath(path$$1, options.addAssignedNodeAtPath(path$$1, this.init));
  };

  LocalVariable.prototype.hasEffectsWhenCalledAtPath = function (path$$1, callOptions, options) {
    if (path$$1.length > MAX_PATH_DEPTH) return true;
    return this.isReassigned || this.init && !options.hasNodeBeenCalledAtPathWithOptions(path$$1, this.init, callOptions) && this.init.hasEffectsWhenCalledAtPath(path$$1, callOptions, options.addCalledNodeAtPathWithOptions(path$$1, this.init, callOptions));
  };
  ...
  LocalVariable.prototype.include = function () {
    if (!this.included) {
      ...
      for (var _i = 0, _a = this.declarations; _i < _a.length; _i++) {
        var declaration = _a[_i]; // If node is a default export, it can save a tree-shaking run to include the full declaration now
...
var ExportDefaultVariable = function (_super) {
  __extends(ExportDefaultVariable, _super);

  function ExportDefaultVariable(name, exportDefaultDeclaration, deoptimizationTracker) {
    var _this = _super.call(this, name, exportDefaultDeclaration, exportDefaultDeclaration.declaration, deoptimizationTracker) || this; // Not initialised during construction
...
var ExternalVariable = function (_super) {
...
function normalize(path$$1) {
  if (path$$1.indexOf('\\') == -1) return path$$1;
...
var ExternalModule = function () {
  ...
  ExternalModule.prototype.setRenderPath = function (options, inputBase) {
    if (options.paths) this.renderPath = typeof options.paths === 'function' ? options.paths(this.id) : options.paths[this.id];
...
function getExportBlock(exports, dependencies, namedExportsMode, interop, compact, mechanism) {
  ...
  var exportBlock = ''; // star exports must always output first for precedence

  dependencies.forEach(function (_a) {
    var name = _a.name,
        reexports = _a.reexports;
    ...
    if (reexports && namedExportsMode) {
      reexports.forEach(function (specifier) {
        if (specifier.imported === 'default' && !isChunk) {
          var exportsNamesOrNamespace = imports && imports.some(function (specifier) {
            return specifier.imported === '*' || specifier.imported !== 'default';
          }) || reexports && reexports.some(function (specifier) {
            return specifier.imported !== 'default' && specifier.imported !== '*';
          });
          if (exportBlock && !compact) exportBlock += '\n';
          if (exportsNamesOrNamespace) exportBlock += "exports." + specifier.reexported + _ + "=" + _ + name + (interop !== false ? '__default' : '.default') + ";";else exportBlock += "exports." + specifier.reexported + _ + "=" + _ + name + ";";
...
function getInteropBlock(dependencies, options, varOrConst) {
  return dependencies.map(function (_a) {
    var name = _a.name,
        exportsNames = _a.exportsNames,
        exportsDefault = _a.exportsDefault,
        namedExportsMode = _a.namedExportsMode;
    if (!namedExportsMode) return;
    if (!exportsDefault || options.interop === false) return null;

    if (exportsNames) {
      if (options.compact) return varOrConst + " " + name + "__default='default'in " + name + "?" + name + "['default']:" + name + ";";
      return varOrConst + " " + name + "__default = 'default' in " + name + " ? " + name + "['default'] : " + name + ";";
    }

    if (options.compact) return name + "=" + name + "&&" + name + ".hasOwnProperty('default')?" + name + "['default']:" + name + ";";
...
function amd(magicString, _a, options) {
  var graph = _a.graph,
      namedExportsMode = _a.namedExportsMode,
      hasExports = _a.hasExports,
      indentString = _a.indentString,
      intro = _a.intro,
      outro = _a.outro,
      dynamicImport = _a.dynamicImport,
      needsAmdModule = _a.needsAmdModule,
      dependencies = _a.dependencies,
      exports = _a.exports,
      isEntryModuleFacade = _a.isEntryModuleFacade;
  warnOnBuiltins(graph, dependencies);
  var deps = dependencies.map(function (m) {
    return "'" + m.id + "'";
  });
  var args = dependencies.map(function (m) {
    return m.name;
  });
  ...
  var params = (amdOptions.id ? "'" + amdOptions.id + "'," + _ : "") + (deps.length ? "[" + deps.join("," + _) + "]," + _ : "");
  var useStrict = options.strict !== false ? _ + "'use strict';" : "";
  var define = amdOptions.define || 'define';
  var wrapperStart = define + "(" + params + "function" + _ + "(" + args.join("," + _) + ")" + _ + "{" + useStrict + n + n; // var foo__default = 'default' in foo ? foo['default'] : foo;
  ...
  return magicString.indent(indentString).append(n + n + '});').prepend(wrapperStart);
}
    ...
    dependencies.forEach(function (_a) {
      ...
      if (!reexports && !imports) {
        ...
        if (!interop || isChunk || !exportsDefault || !namedExportsMode) {
          ...
          needsInterop = true;
          if (exportsNames) importBlock += name + "=require('" + id + "')," + name + "__default=_interopDefault(" + name + ")";else importBlock += name + "=_interopDefault(require('" + id + "'))";
        }
      }
    });
    if (importBlock.length) importBlock += ';';
  } else {
    importBlock = dependencies.map(function (_a) {
      var id = _a.id,
          namedExportsMode = _a.namedExportsMode,
          isChunk = _a.isChunk,
          name = _a.name,
      ...
      if (!interop || isChunk || !exportsDefault || !namedExportsMode) return varOrConst + " " + name + " = require('" + id + "');";
      needsInterop = true;
      if (exportsNames) return varOrConst + " " + name + " = require('" + id + "');" + ("\n" + varOrConst + " " + name + "__default = _interopDefault(" + name + ");");
...
function esm(magicString, _a, options) {
  ...
  var importBlock = dependencies.map(function (_a) {
    ...
    if (imports) {
      var defaultImport_1 = imports.find(function (specifier) {
        return specifier.imported === 'default';
      });
      var starImport_1 = imports.find(function (specifier) {
      ...
      if (defaultImport_1 && imports.length === 1) {
        ...
        output += "import " + (defaultImport_1 ? defaultImport_1.local + "," + _ : '') + "{" + _ + imports.filter(function (specifier) {
          return specifier !== defaultImport_1 && specifier !== starImport_1;
    ...
    if (reexports) {
      if (imports) output += n;
      var starExport_1 = reexports.find(function (specifier) {
        return specifier.reexported === '*';
      });
      var namespaceReexport_1 = reexports.find(function (specifier) {
      ...
      if (namespaceReexport_1) {
        if (!imports || !imports.some(function (specifier) {
          return specifier.imported === '*' && specifier.local === name;
        })) output += "import" + _ + "*" + _ + "as " + name + " from" + _ + "'" + id + "';" + n;
        output += "export" + _ + "{" + _ + (name === namespaceReexport_1.reexported ? name : name + " as " + namespaceReexport_1.reexported) + " };";
      ...
      output += "export" + _ + "{" + _ + reexports.filter(function (specifier) {
        return specifier !== starExport_1 && specifier !== namespaceReexport_1;
      }).map(function (specifier) {
...
function setupNamespace(name, root, forAssignment, globals, compact) {
  ...
  if (forAssignment) {
    return parts.map(function (part) {
      return acc += property(part), "" + acc + _ + "=" + _ + acc + _ + "||" + _ + "{}";
    }).concat("" + acc + property(last)).join("," + _);
  } else {
    return parts.map(function (part) {
      return acc += property(part), "" + acc + _ + "=" + _ + acc + _ + "||" + _ + "{}" + (compact ? '' : ';');
...
function iife(magicString, _a, options) {
  ...
  var deps = external.map(function (dep) {
    return dep.globalName || 'null';
  });
  ...
  if (hasExports && !extend) {
    wrapperIntro = (isNamespaced ? thisProp(name) : graph.varOrConst + " " + name) + (_ + "=" + _ + wrapperIntro);
  }

  if (isNamespaced) {
    wrapperIntro = setupNamespace(name, 'this', false, options.globals, options.compact) + wrapperIntro;
  }
...
function getStarExcludes(_a) {
  var dependencies = _a.dependencies,
      exports = _a.exports;
  var starExcludes = new Set(exports.map(function (expt) {
    return expt.exported;
  }));
  if (!starExcludes.has('default')) starExcludes.add('default'); // also include reexport names

  dependencies.forEach(function (_a) {
    var reexports = _a.reexports;
    if (reexports) reexports.forEach(function (reexport) {
...
function system(magicString, _a, options) {
  var graph = _a.graph,
      ...
      dependencies = _a.dependencies,
      exports = _a.exports,
      usesTopLevelAwait = _a.usesTopLevelAwait;
  var n = options.compact ? '' : '\n';

  var _ = options.compact ? '' : ' ';

  var dependencyIds = dependencies.map(function (m) {
    return "'" + m.id + "'";
  });
  var importBindings = [];
  var starExcludes;
  ...
  dependencies.forEach(function (_a) {
    ...
    if (reexports) {
      var createdSetter_1 = false; // bulk-reexport form

      if (reexports.length > 1 || reexports.length === 1 && (reexports[0].reexported === '*' || reexports[0].imported === '*')) {
        // star reexports
        reexports.forEach(function (specifier) {
          if (specifier.reexported !== '*') return; // need own exports list for deduping in star export case
  ...
  var hoistedExports = [];
  var hoistedExportObjs = exports.filter(function (expt) {
    return expt.hoisted || expt.uninitialized;
  });
  ...
  var starExcludesSection = !starExcludes ? '' : "" + n + t + varOrConst + " _starExcludes" + _ + "=" + _ + "{" + _ + Array.from(starExcludes).join(":" + _ + "1," + _) + (starExcludes.size ? ":" + _ + "1" : '') + _ + "};";
  var importBindingsSection = importBindings.length ? "" + n + t + "var " + importBindings.join("," + _) + ";" : '';
  ...
  wrapperStart += t + "return" + _ + "{" + (setters.length ? "" + n + t + t + "setters:" + _ + "[" + setters.map(function (s) {
    return s ? "function" + _ + "(module)" + _ + "{" + n + t + t + t + s + n + t + t + "}" : "function" + _ + "()" + _ + "{}";
  }).join("," + _) + "]," : '') + n;
  wrapperStart += "" + t + t + "execute:" + _ + (usesTopLevelAwait ? "async" + _ : '') + "function" + _ + "()" + _ + "{" + n + n;
  if (hoistedExports.length) wrapperStart += "" + t + t + t + hoistedExports.join("" + n + t + t + t) + n + n;
...
function umd(magicString, _a, options) {
  ...
  warnOnBuiltins(graph, dependencies);
  var amdDeps = dependencies.map(function (m) {
    return "'" + m.id + "'";
  });
  var cjsDeps = dependencies.map(function (m) {
    return "require('" + m.id + "')";
  });
  var trimmed = trimEmptyImports(dependencies);
  var globalDeps = trimmed.map(function (module) {
    return globalProp(module.globalName);
  });
  ...
  var amdOptions = options.amd || {};
  var amdParams = (amdOptions.id ? "'" + amdOptions.id + "'," + _ : "") + (amdDeps.length ? "[" + amdDeps.join("," + _) + "]," + _ : "");
  var define = amdOptions.define || 'define';
  var cjsExport = !namedExportsMode && hasExports ? "module.exports" + _ + "=" + _ : "";
  var defaultExport = !namedExportsMode && hasExports ? "" + setupNamespace(options.name, 'global', true, options.globals, options.compact) + _ + "=" + _ : '';
  ...
  wrapperIntro += "}(this," + _ + "(function" + _ + "(" + args + ")" + _ + "{" + useStrict + n; // var foo__default = 'default' in foo ? foo['default'] : foo;
...
var Scope = function () {
  ...
  Scope.prototype.deshadow = function (names, children) {
    ...
    for (var _i = 0, _a = Object.keys(this.variables); _i < _a.length; _i++) {
      var key = _a[_i];
      var declaration = this.variables[key]; // we can disregard exports.foo etc
...
var ExecutionPathOptions = function () {
...
function getAndCreateKeys(esTreeNode) {
  keys[esTreeNode.type] = Object.keys(esTreeNode).filter(function (key) {
...
var NodeBase = function () {
  function NodeBase(esTreeNode, // we need to pass down the node constructors to avoid a circular dependency
  parent, parentScope) {
  ...
  NodeBase.prototype.locate = function () {
    ...
    var location = locate(this.context.code, this.start, {
      offsetLine: 1
    });
  ...
  NodeBase.prototype.parseNode = function (esTreeNode) {
    for (var _i = 0, _a = Object.keys(esTreeNode); _i < _a.length; _i++) {
      var key = _a[_i]; // That way, we can override this function to add custom initialisation and then call super.parseNode
...
var ClassDeclaration$1 = function (_super) {
...
var ArgumentsVariable = function (_super) {
...
var ReplaceableInitializationVariable = function (_super) {
...
var ParameterVariable = function (_super) {
...
var ParameterScope = function (_super) {
...
var ReturnValueScope = function (_super) {
...
var FunctionScope = function (_super) {
  ...
  FunctionScope.prototype.getOptionsWhenCalledWith = function (_a, options) {
    ...
    var args = _a.args,
        withNew = _a.withNew;
    return options.replaceVariableInit(this.variables.this, withNew ? new UnknownObjectExpression() : UNKNOWN_EXPRESSION).setArgumentsVariables(args.map(function (parameter, index) {
...
var FunctionDeclaration$1 = function (_super) {
...
function isReference(node, parent) {
  ...
  if (node.type === 'Identifier') {
    ...
    if (parent.type === 'Property') return parent.computed || node === parent.value; // disregard the `bar` in `export { foo as bar }`
...
var Identifier$1 = function (_super) {
  ...
  Identifier$$1.prototype.bind = function () {
    ...
    if (this.variable !== null && this.variable.isLocal && this.variable.additionalInitializers !== null) {
  ...
  Identifier$$1.prototype.initialise = function () {
    this.included = false;
    this.bound = false; // To avoid later shape mutations
  ...
  Identifier$$1.prototype.deoptimizePath = function (path$$1) {
    ...
    if (this.variable !== null) {
      if (path$$1.length === 0 && this.name in this.context.imports && !this.scope.contains(this.name)) {
  ...
  Identifier$$1.prototype.render = function (code, _options, _a) {
    var _b = _a === void 0 ? BLANK : _a,
        renderedParentType = _b.renderedParentType,
        isCalleeOfRenderedParent = _b.isCalleeOfRenderedParent;
...
var WHITESPACE = /\s/; // The header ends at the first non-white-space after "default"
...
var ExportDefaultDeclaration$1 = function (_super) {
  ...
  ExportDefaultDeclaration$$1.prototype.bind = function () {
    ...
    if (this.declarationName && (isFunctionDeclaration(this.declaration) || isClassDeclaration(this.declaration) || isIdentifier(this.declaration))) {
  ...
  ExportDefaultDeclaration$$1.prototype.initialise = function () {
    this.included = false;
    this.declarationName = this.declaration.id && this.declaration.id.name || this.declaration.name;
  ...
  ExportDefaultDeclaration$$1.prototype.renderNamedDeclaration = function (code, declarationStart, declarationKeyword, needsId, options) {
    var name = this.variable.getName(); // Remove `export default`
    ...
    if (options.format === 'system' && isClassDeclaration(this.declaration) && this.variable.exportName) {
  ...
  ExportDefaultDeclaration$$1.prototype.renderVariableDeclaration = function (code, declarationStart, options) {
    var systemBinding = options.format === 'system' && this.variable.exportName ? "exports('" + this.variable.exportName + "', " : '';
...
var ArrayExpression$1 = function (_super) {
  ...
  ArrayExpression.prototype.getReturnExpressionWhenCalledAtPath = function (path$$1) {
    if (path$$1.length !== 1) return UNKNOWN_EXPRESSION;
    return getMemberReturnExpressionWhenCalled(arrayMembers, path$$1[0]);
  };
...
var ArrayPattern$1 = function (_super) {
  ...
  ArrayPattern.prototype.hasEffectsWhenAssignedAtPath = function (path$$1, options) {
    ...
    for (var _i = 0, _a = this.elements; _i < _a.length; _i++) {
      var element = _a[_i];
      if (element !== null && element.hasEffectsWhenAssignedAtPath(EMPTY_PATH, options)) return true;
    }
...
var BlockStatement$1 = function (_super) {
...
var ArrowFunctionExpression$1 = function (_super) {
...
var AssignmentExpression$1 = function (_super) {
  ...
  AssignmentExpression.prototype.bind = function () {
    ...
    this.left.deoptimizePath(EMPTY_PATH); // We cannot propagate mutations of the new binding to the old binding with certainty

    this.right.deoptimizePath(UNKNOWN_PATH);
  };

  AssignmentExpression.prototype.hasEffects = function (options) {
    return this.right.hasEffects(options) || this.left.hasEffects(options) || this.left.hasEffectsWhenAssignedAtPath(EMPTY_PATH, options);
  };
...
var AssignmentPattern$1 = function (_super) {
...
var AwaitExpression$1 = function (_super) {
  ...
  AwaitExpression.prototype.include = function () {
    ...
    if (!this.context.usesTopLevelAwait) {
      ...
      do {
        if (parent instanceof FunctionNode || parent instanceof ArrowFunctionExpression$1) return;
...
var BinaryExpression$1 = function (_super) {
  ...
  BinaryExpression.prototype.getLiteralValueAtPath = function (path$$1, recursionTracker, origin) {
    if (path$$1.length > 0) return UNKNOWN_VALUE;
...
var BreakStatement$1 = function (_super) {
...
var ImmutableEntityPathTracker = function () {
  function ImmutableEntityPathTracker(existingEntityPaths) {
    if (existingEntityPaths === void 0) {
...
var CallExpression$1 = function (_super) {
  ...
  CallExpression.prototype.bind = function () {
    ...
    for (var _i = 0, _a = this.arguments; _i < _a.length; _i++) {
      var argument = _a[_i]; // This will make sure all properties of parameters behave as "unknown"
  ...
  CallExpression.prototype.getLiteralValueAtPath = function (path$$1, recursionTracker, origin) {
    ...
    if (this.returnExpression === UNKNOWN_EXPRESSION || recursionTracker.isTracked(this.returnExpression, path$$1)) {
      return UNKNOWN_VALUE;
    }
  ...
  CallExpression.prototype.hasEffects = function (options) {
    ...
    return this.callee.hasEffects(options) || this.callee.hasEffectsWhenCalledAtPath(EMPTY_PATH, this.callOptions, options.getHasEffectsWhenCalledOptions());
  };

  CallExpression.prototype.hasEffectsWhenAccessedAtPath = function (path$$1, options) {
    return path$$1.length > 0 && !options.hasReturnExpressionBeenAccessedAtPath(path$$1, this) && this.returnExpression.hasEffectsWhenAccessedAtPath(path$$1, options.addAccessedReturnExpressionAtPath(path$$1, this));
  };

  CallExpression.prototype.hasEffectsWhenAssignedAtPath = function (path$$1, options) {
    return path$$1.length === 0 || !options.hasReturnExpressionBeenAssignedAtPath(path$$1, this) && this.returnExpression.hasEffectsWhenAssignedAtPath(path$$1, options.addAssignedReturnExpressionAtPath(path$$1, this));
  };

  CallExpression.prototype.hasEffectsWhenCalledAtPath = function (path$$1, callOptions, options) {
    if (options.hasReturnExpressionBeenCalledAtPath(path$$1, this)) return false;
    var innerOptions = options.addCalledReturnExpressionAtPath(path$$1, this);
    return this.hasEffects(innerOptions) || this.returnExpression.hasEffectsWhenCalledAtPath(path$$1, callOptions, innerOptions);
  };
...
var CatchScope = function (_super) {
...
var CatchClause$1 = function (_super) {
...
var ClassBody$1 = function (_super) {
  ...
  ClassBody.prototype.hasEffectsWhenCalledAtPath = function (path$$1, callOptions, options) {
    ...
    return this.classConstructor !== null && this.classConstructor.hasEffectsWhenCalledAtPath(EMPTY_PATH, callOptions, options);
  };
...
var ClassExpression$1 = function (_super) {
...
var MultiExpression = function () {
  ...
  MultiExpression.prototype.hasEffectsWhenAccessedAtPath = function (path$$1, options) {
    for (var _i = 0, _a = this.expressions; _i < _a.length; _i++) {
      var expression = _a[_i];
      if (expression.hasEffectsWhenAccessedAtPath(path$$1, options)) return true;
    }
  ...
  MultiExpression.prototype.hasEffectsWhenAssignedAtPath = function (path$$1, options) {
    for (var _i = 0, _a = this.expressions; _i < _a.length; _i++) {
      var expression = _a[_i];
      if (expression.hasEffectsWhenAssignedAtPath(path$$1, options)) return true;
    }
  ...
  MultiExpression.prototype.hasEffectsWhenCalledAtPath = function (path$$1, callOptions, options) {
    for (var _i = 0, _a = this.expressions; _i < _a.length; _i++) {
      var expression = _a[_i];
      if (expression.hasEffectsWhenCalledAtPath(path$$1, callOptions, options)) return true;
    }
...
var ConditionalExpression$1 = function (_super) {
  ...
  ConditionalExpression.prototype.getReturnExpressionWhenCalledAtPath = function (path$$1, recursionTracker, origin) {
    if (!this.isBranchResolutionAnalysed) this.analyseBranchResolution();
    if (this.usedBranch === null) return new MultiExpression([this.consequent.getReturnExpressionWhenCalledAtPath(path$$1, recursionTracker, origin), this.alternate.getReturnExpressionWhenCalledAtPath(path$$1, recursionTracker, origin)]);
    this.expressionsToBeDeoptimized.push(origin);
  ...
  ConditionalExpression.prototype.hasEffectsWhenAccessedAtPath = function (path$$1, options) {
    ...
    if (this.usedBranch === null) {
      return this.consequent.hasEffectsWhenAccessedAtPath(path$$1, options) || this.alternate.hasEffectsWhenAccessedAtPath(path$$1, options);
    }
  ...
  ConditionalExpression.prototype.hasEffectsWhenAssignedAtPath = function (path$$1, options) {
    ...
    if (this.usedBranch === null) {
      return this.consequent.hasEffectsWhenAssignedAtPath(path$$1, options) || this.alternate.hasEffectsWhenAssignedAtPath(path$$1, options);
    }
  ...
  ConditionalExpression.prototype.hasEffectsWhenCalledAtPath = function (path$$1, callOptions, options) {
    if (this.usedBranch === null) {
      return this.consequent.hasEffectsWhenCalledAtPath(path$$1, callOptions, options) || this.alternate.hasEffectsWhenCalledAtPath(path$$1, callOptions, options);
    }
...
var DoWhileStatement$1 = function (_super) {
...
var EmptyStatement$1 = function (_super) {
...
var ExportAllDeclaration$1 = function (_super) {
...
var ExportNamedDeclaration$1 = function (_super) {
...
var ExpressionStatement$1 = function (_super) {
...
var ForInStatement$1 = function (_super) {
  ...
  ForInStatement$$1.prototype.hasEffects = function (options) {
    return this.left && (this.left.hasEffects(options) || this.left.hasEffectsWhenAssignedAtPath(EMPTY_PATH, options)) || this.right && this.right.hasEffects(options) || this.body.hasEffects(options.setIgnoreBreakStatements());
  };
...
var ForOfStatement$1 = function (_super) {
...
var ForStatement$1 = function (_super) {
...
var FunctionExpression$1 = function (_super) {
...
var IfStatement$1 = function (_super) {
  ...
  IfStatement.prototype.hasEffects = function (options) {
    ...
    if (this.testValue === UNKNOWN_VALUE) {
      return this.consequent.hasEffects(options) || this.alternate !== null && this.alternate.hasEffects(options);
    }

    return this.testValue ? this.consequent.hasEffects(options) : this.alternate !== null && this.alternate.hasEffects(options);
  };
  ...
  IfStatement.prototype.include = function () {
    ...
    if ((this.testValue === UNKNOWN_VALUE || this.testValue) && this.consequent.shouldBeIncluded()) {
      this.consequent.include();
    }

    if (this.alternate !== null && (this.testValue === UNKNOWN_VALUE || !this.testValue) && this.alternate.shouldBeIncluded()) {
      this.alternate.include();
    }
  };
  ...
  IfStatement.prototype.render = function (code, options) {
    ...
    if (!this.test.included && (this.testValue ? this.alternate === null || !this.alternate.included : !this.consequent.included)) {
...
var Import$1 = function (_super) {
  ...
  Import.prototype.renderFinalResolution = function (code, resolution) {
    ...
    if (this.rendered) code.overwrite(this.parent.arguments[0].start, this.parent.arguments[0].end, resolution);
  };
...
var ImportDeclaration$1 = function (_super) {
...
var LabeledStatement$1 = function (_super) {
...
var LogicalExpression$1 = function (_super) {
  ...
  LogicalExpression.prototype.getReturnExpressionWhenCalledAtPath = function (path$$1, recursionTracker, origin) {
    if (!this.isBranchResolutionAnalysed) this.analyseBranchResolution();
    if (this.usedBranch === null) return new MultiExpression([this.left.getReturnExpressionWhenCalledAtPath(path$$1, recursionTracker, origin), this.right.getReturnExpressionWhenCalledAtPath(path$$1, recursionTracker, origin)]);
    this.expressionsToBeDeoptimized.push(origin);
    return this.usedBranch.getReturnExpressionWhenCalledAtPath(path$$1, recursionTracker, origin);
  };
  ...
  LogicalExpression.prototype.hasEffectsWhenAccessedAtPath = function (path$$1, options) {
    ...
    if (this.usedBranch === null) {
      return this.left.hasEffectsWhenAccessedAtPath(path$$1, options) || this.right.hasEffectsWhenAccessedAtPath(path$$1, options);
    }
  ...
  LogicalExpression.prototype.hasEffectsWhenAssignedAtPath = function (path$$1, options) {
    ...
    if (this.usedBranch === null) {
      return this.left.hasEffectsWhenAssignedAtPath(path$$1, options) || this.right.hasEffectsWhenAssignedAtPath(path$$1, options);
    }
  ...
  LogicalExpression.prototype.hasEffectsWhenCalledAtPath = function (path$$1, callOptions, options) {
    if (this.usedBranch === null) {
      return this.left.hasEffectsWhenCalledAtPath(path$$1, callOptions, options) || this.right.hasEffectsWhenCalledAtPath(path$$1, callOptions, options);
    }
...
function isPlainName(name) {
  // not starting with "./", "/". "../"
  if (name[0] === '/' || name[1] === '.' && (name[2] === '/' || name[2] === '.' && name[3] === '/')) return false; // not a URL
...
var MemberExpression$1 = function (_super) {
  ...
  MemberExpression$$1.prototype.hasEffects = function (options) {
    return this.property.hasEffects(options) || this.object.hasEffects(options) || this.context.propertyReadSideEffects && this.object.hasEffectsWhenAccessedAtPath([this.propertyKey], options);
  };
  ...
  MemberExpression$$1.prototype.resolveNamespaceVariables = function (baseVariable, path$$1) {
    ...
    var variable = baseVariable.isExternal ? baseVariable.module.traceExport(exportName) : baseVariable.context.traceExport(exportName);

    if (!variable) {
      var fileName = baseVariable.isExternal ? baseVariable.module.id : baseVariable.context.fileName;
      this.context.warn({
        code: 'MISSING_EXPORT',
        missing: exportName,
...
var MetaProperty$1 = function (_super) {
  ...
  MetaProperty.prototype.renderFinalMechanism = function (code, chunkId, format, compact) {
    ...
    if (this.parent instanceof MemberExpression$1 === false) return false;
    var parent = this.parent;
    var importMetaProperty;
    if (parent.property instanceof Identifier$1) importMetaProperty = parent.property.name;else if (parent.property instanceof Literal$1 && typeof parent.property.value === 'string') importMetaProperty = parent.property.value;else return false; // support import.meta.ROLLUP_ASSET_URL_[ID]
    ...
    if (format === 'system') {
      ...
      var importMetaUrlMechanism = (compact ? importMetaUrlMechanismsCompact : importMetaUrlMechanisms)[format];
...
var MethodDefinition$1 = function (_super) {
...
var NewExpression$1 = function (_super) {
...
var SpreadElement$1 = function (_super) {
  ...
  SpreadElement.prototype.bind = function () {
    _super.prototype.bind.call(this); // Only properties of properties of the argument could become subject to reassignment
...
var ObjectExpression$1 = function (_super) {
  ...
  ObjectExpression$$1.prototype.getLiteralValueAtPath = function (path$$1, recursionTracker, origin) {
    if (this.propertyMap === null) this.buildPropertyMap();
    var key = path$$1[0];
    if (path$$1.length === 0 || this.hasUnknownDeoptimizedProperty || typeof key !== 'string' || this.deoptimizedPaths[key]) return UNKNOWN_VALUE;

    if (path$$1.length === 1 && !this.propertyMap[key] && this.unmatchablePropertiesRead.length === 0) {
    ...
    if (!this.propertyMap[key] || this.propertyMap[key].exactMatchRead === null || this.propertyMap[key].propertiesRead.length > 1) return UNKNOWN_VALUE;
  ...
  ObjectExpression$$1.prototype.getReturnExpressionWhenCalledAtPath = function (path$$1, recursionTracker, origin) {
    if (this.propertyMap === null) this.buildPropertyMap();
    var key = path$$1[0];
    if (path$$1.length === 0 || this.hasUnknownDeoptimizedProperty || typeof key !== 'string' || this.deoptimizedPaths[key]) return UNKNOWN_EXPRESSION;
    if (path$$1.length === 1 && objectMembers[key] && this.unmatchablePropertiesRead.length === 0 && (!this.propertyMap[key] || this.propertyMap[key].exactMatchRead === null)) return getMemberReturnExpressionWhenCalled(objectMembers, key);
    if (!this.propertyMap[key] || this.propertyMap[key].exactMatchRead === null || this.propertyMap[key].propertiesRead.length > 1) return UNKNOWN_EXPRESSION;
  ...
  ObjectExpression$$1.prototype.hasEffectsWhenAccessedAtPath = function (path$$1, options) {
    if (path$$1.length === 0) return false;
    var key = path$$1[0];
    if (path$$1.length > 1 && (this.hasUnknownDeoptimizedProperty || typeof key !== 'string' || this.deoptimizedPaths[key] || !this.propertyMap[key] || this.propertyMap[key].exactMatchRead === null)) return true;
    var subPath = path$$1.slice(1);

    for (var _i = 0, _a = typeof key !== 'string' ? this.properties : this.propertyMap[key] ? this.propertyMap[key].propertiesRead : []; _i < _a.length; _i++) {
      var property = _a[_i];
  ...
  ObjectExpression$$1.prototype.hasEffectsWhenAssignedAtPath = function (path$$1, options) {
    ...
    for (var _i = 0, _a = typeof key !== 'string' ? this.properties : path$$1.length > 1 ? this.propertyMap[key].propertiesRead : this.propertyMap[key] ? this.propertyMap[key].propertiesSet : []; _i < _a.length; _i++) {
      var property = _a[_i];
  ...
  ObjectExpression$$1.prototype.hasEffectsWhenCalledAtPath = function (path$$1, callOptions, options) {
    var key = path$$1[0];
    if (path$$1.length === 0 || this.hasUnknownDeoptimizedProperty || typeof key !== 'string' || this.deoptimizedPaths[key] || (this.propertyMap[key] ? !this.propertyMap[key].exactMatchRead : path$$1.length > 1 || !objectMembers[key])) return true;
    var subPath = path$$1.slice(1);
    ...
    if (path$$1.length === 1 && objectMembers[key]) return hasMemberEffectWhenCalled(objectMembers, key, this.included, callOptions, options);
    return false;
  };
  ...
  ObjectExpression$$1.prototype.deoptimizePath = function (path$$1) {
    ...
    if (path$$1.length === 1) {
      ...
      if (!this.deoptimizedPaths[key]) {
        this.deoptimizedPaths[key] = true; // we only deoptimizeCache exact matches as in all other cases,
    ...
    for (var _b = 0, _c = typeof key === 'string' ? this.propertyMap[key] ? this.propertyMap[key].propertiesRead : [] : this.properties; _b < _c.length; _b++) {
      var property = _c[_b];
...
var ObjectPattern$1 = function (_super) {
...
var Property$1 = function (_super) {
  ...
  Property.prototype.hasEffectsWhenAccessedAtPath = function (path$$1, options) {
    if (this.kind === 'get') {
      return this.value.hasEffectsWhenCalledAtPath(EMPTY_PATH, this.accessorCallOptions, options.getHasEffectsWhenCalledOptions()) || path$$1.length > 0 && this.returnExpression.hasEffectsWhenAccessedAtPath(path$$1, options);
    }
  ...
  Property.prototype.hasEffectsWhenAssignedAtPath = function (path$$1, options) {
    ...
    if (this.kind === 'set') {
      return path$$1.length > 0 || this.value.hasEffectsWhenCalledAtPath(EMPTY_PATH, this.accessorCallOptions, options.getHasEffectsWhenCalledOptions());
    }
...
var RestElement$1 = function (_super) {
...
var ReturnStatement$1 = function (_super) {
...
var SequenceExpression$1 = function (_super) {
  ...
  SequenceExpression.prototype.hasEffectsWhenAccessedAtPath = function (path$$1, options) {
    return path$$1.length > 0 && this.expressions[this.expressions.length - 1].hasEffectsWhenAccessedAtPath(path$$1, options);
  };

  SequenceExpression.prototype.hasEffectsWhenAssignedAtPath = function (path$$1, options) {
    return path$$1.length === 0 || this.expressions[this.expressions.length - 1].hasEffectsWhenAssignedAtPath(path$$1, options);
  };
  ...
  SequenceExpression.prototype.deoptimizePath = function (path$$1) {
    if (path$$1.length > 0) this.expressions[this.expressions.length - 1].deoptimizePath(path$$1);
  };
...
var SwitchCase$1 = function (_super) {
  ...
  SwitchCase.prototype.render = function (code, options) {
    if (this.consequent.length) {
      ...
      var testEnd = this.test ? this.test.end : findFirstOccurrenceOutsideComment(code.original, 'default', this.start) + 7;
...
var SwitchStatement$1 = function (_super) {
...
var TaggedTemplateExpression$1 = function (_super) {
  ...
  TaggedTemplateExpression$$1.prototype.hasEffects = function (options) {
    return _super.prototype.hasEffects.call(this, options) || this.tag.hasEffectsWhenCalledAtPath(EMPTY_PATH, this.callOptions, options.getHasEffectsWhenCalledOptions());
  };
...
var TemplateElement$1 = function (_super) {
...
var TemplateLiteral$1 = function (_super) {
...
var ThisExpression$1 = function (_super) {
...
var ThrowStatement$1 = function (_super) {
...
var UnaryExpression$1 = function (_super) {
  ...
  UnaryExpression.prototype.hasEffects = function (options) {
    return this.argument.hasEffects(options) || this.operator === 'delete' && this.argument.hasEffectsWhenAssignedAtPath(EMPTY_PATH, options);
  };
...
var UpdateExpression$1 = function (_super) {
  ...
  UpdateExpression.prototype.hasEffects = function (options) {
    return this.argument.hasEffects(options) || this.argument.hasEffectsWhenAssignedAtPath(EMPTY_PATH, options);
  };
...
var VariableDeclaration$1 = function (_super) {
  ...
  VariableDeclaration$$1.prototype.renderReplacedDeclarations = function (code, options, _a) {
    var _b = _a.start,
        start = _b === void 0 ? this.start : _b,
        _c = _a.end,
        end = _c === void 0 ? this.end : _c,
        isNoStatement = _a.isNoStatement;
    var separatedNodes = getCommaSeparatedNodesWithBoundaries(this.declarations, code, this.start + this.kind.length, this.end - (code.original.charCodeAt(this.end - 1) === 59
    /*";"*/
    ? 1 : 0));
    ...
    for (var _i = 0, separatedNodes_1 = separatedNodes; _i < separatedNodes_1.length; _i++) {
      var _d = separatedNodes_1[_i],
          node = _d.node,
          start_1 = _d.start,
          separator = _d.separator,
          contentEnd = _d.contentEnd,
          end_1 = _d.end;
      ...
      if (isIdentifier(node.id) && isReassignedExportsMember(node.id.variable)) {
        ...
        if (options.format === 'system' && node.init !== null && isIdentifier(node.id) && node.id.variable.exportName) {
...
var VariableDeclarator$1 = function (_super) {
...
var WhileStatement$1 = function (_super) {
...
var YieldExpression$1 = function (_super) {
...
var ModuleScope = function (_super) {
  ...
  ModuleScope.prototype.deshadow = function (names, children) {
    ...
    for (var _i = 0, _a = Object.keys(this.context.imports); _i < _a.length; _i++) {
      ...
      if (importDescription.module.isExternal || this.context.isCrossChunkImport(importDescription)) continue;
      ...
      if (importDescription.name !== '*') {
        ...
        if (importDescription.name !== 'default' && importDescription.name !== importName) localNames.add(importDescription.name);
      }
    }
...
var NamespaceVariable = function (_super) {
  __extends(NamespaceVariable, _super);

  function NamespaceVariable(context, module) {
    var _this = _super.call(this, context.getModuleName()) || this; // Not initialised during construction
  ...
  NamespaceVariable.prototype.renderBlock = function (options) {
    ...
    var output = this.context.varOrConst + " " + name + " = " + (options.namespaceToStringTag ? "{" + n + members.join("," + n) + n + "};" : callee + "({" + n + members.join("," + n) + n + "});");
...
var getElapsedMemory = function () {
  return 0;
};

var timers = {};

var normalizeHrTime = function (time) {
...
function setTimeHelpers() {
  if (typeof process !== 'undefined' && typeof process.hrtime === 'function') {
    ...
    getElapsedTime = function (previous) {
      return normalizeHrTime(process.hrtime(previous));
    };
  } else if (typeof performance !== 'undefined' && typeof performance.now === 'function') {
    getStartTime = performance.now.bind(performance);

    getElapsedTime = function (previous) {
      return performance.now() - previous;
    };
  }

  if (typeof process !== 'undefined' && typeof process.memoryUsage === 'function') {
    getStartMemory = function () {
    ...
    getElapsedMemory = function (previous) {
      return getStartMemory() - previous;
    };
  }
}
      ...
      timedPlugin[hook] = function () {
        ...
        if (result && typeof result.then === 'function') {
          ...
          result.then(function () {
            return timeEnd(timerLabel_1 + " (async)", 4);
          });
        }
...
function tryParse(module, parse, acornOptions) {
  try {
    return parse(module.code, __assign({}, defaultAcornOptions, acornOptions, {
...
var Module = function () {
  function Module(graph, id) {
    ...
    this.sources = [];
    this.dependencies = []; // imports and exports, indexed by local name
  ...
  Module.prototype.setSource = function (_a) {
    ...
    var code = _a.code,
        ...
        transformDependencies = _a.transformDependencies,
        customTransformCache = _a.customTransformCache;
    this.code = code;
    ...
    this.resolvedIds = resolvedIds || Object.create(null); // By default, `id` is the filename. Custom resolvers and loaders
    ...
    this.astContext = {
      ...
      getModuleExecIndex: function () {
        return _this.execIndex;
      },
      getModuleName: this.basename.bind(this),
      imports: this.imports,
      includeNamespace: this.includeNamespace.bind(this),
      isCrossChunkImport: function (importDescription) {
        return importDescription.module.chunk !== _this.chunk;
      },
      ...
      requestTreeshakingPass: function () {
        return _this.needsTreeshakingPass = true;
      },
  ...
  Module.prototype.error = function (props, pos) {
    if (pos !== undefined) {
      props.pos = pos;
      var location = locate(this.code, pos, {
        offsetLine: 1
      });
  ...
  Module.prototype.traceExport = function (name, isExportAllSearch) {
    if (name[0] === '*') {
      // namespace
      if (name.length === 1) {
        return this.getOrCreateNamespace(); // export * from 'external'
      } else {
  ...
  Module.prototype.shimMissingExport = function (name) {
    ...
    if (!this.exports[name]) this.graph.warn({
    ...
    for (var _i = 0, _a = this.mappings; _i < _a.length; _i++) {
      ...
      for (var _b = 0, line_1 = line; _b < line_1.length; _b++) {
        ...
        if (traced) {
          ...
          var tracedSegment = [segment[0], sourceIndex, traced.line, traced.column];
...
function collapseSourcemaps(bundle, file, map, modules, bundleSourcemapChain) {
  ...
  if (file) {
    ...
    sources = sources.map(function (source) {
      return relative(directory_2, source);
    });
    file = basename(file);
  }
...
function renderChunk(_a) {
  ...
  var renderChunkReducer = function (code, result, plugin) {
    ...
    if (map && typeof map.mappings === 'string') map.mappings = decode(map.mappings); // strict null check allows 'null' maps to not be pushed to the chain, while 'undefined' gets the missing map warning

    if (map !== null) sourcemapChain.push(map || {
      missing: true,
      plugin: plugin.name
    });
    return result.code;
  ...
  return graph.pluginDriver.hookReduceArg0('renderChunk', [code, renderChunk, options], renderChunkReducer).then(function (code) {
    inRenderChunk = false;
...
var Chunk$1 = function () {
  function Chunk(graph, orderedModules) {
    ...
    this.id = undefined;
    this.linked = false; // this represents the chunk module wrappings
    // which form the output dependency graph

    this.imports = new Map(); // module can be in or out of chunk
    // if module is out of the chunk then it is a reexport

    this.exports = new Map();
    this.exportNames = Object.create(null);
    this.dependencies = undefined; // an entry module chunk is a chunk that exactly exports the exports of
    ...
    if (this.entryModule) this.name = makeLegal(basename(this.entryModule.chunkAlias || this.orderedModules[0].chunkAlias || this.entryModule.id));else this.name = '__chunk_' + ++graph.curChunkIndex;
  }

  Chunk.prototype.getImportIds = function () {
    return this.dependencies.map(function (module) {
      return module.id;
    });
  };
  ...
  Chunk.prototype.linkModule = function (module) {
    ...
    for (var _d = 0, _e = module.dynamicImportResolutions; _d < _e.length; _d++) {
      ...
      if (resolution instanceof Module && resolution.chunk === this) resolution.getOrCreateNamespace().include();
    }
  ...
  Chunk.prototype.populateEntryExports = function (preserveModules) {
    ...
    for (var _i = 0, entryExportEntries_1 = entryExportEntries; _i < entryExportEntries_1.length; _i++) {
      var _a = entryExportEntries_1[_i],
          index = _a[0],
          exportName = _a[1];
      ...
      var existingExport = this.exportNames[exportName]; // tainted entryModule boundary
    ...
    var this_1 = this; // tainted if we've already exposed something not corresponding to entry exports
    ...
    if (preserveModules || this.isEntryModuleFacade) {
      for (var _d = 0, entryExportEntries_2 = entryExportEntries; _d < entryExportEntries_2.length; _d++) {
        var _e = entryExportEntries_2[_d],
            index = _e[0],
            exportName = _e[1];
  ...
  Chunk.prototype.traceImport = function (exportName, module) {
    ...
    if (!traced) return; // namespace variable can indicate multiple imports

    if (isNamespaceVariable(traced.variable)) {
      var namespaceVariables = traced.variable.originals || traced.variable.module.declarations;
    ...
    if (traced.module instanceof Module) traced.module.chunk.exports.set(traced.variable, traced.module);
    return traced;
  ...
  Chunk.prototype.traceExport = function (name, module) {
    if (name === '*' || module instanceof ExternalModule) {
      return {
        variable: module.traceExport(name),
        module: module
      };
    }
    ...
    if (module.chunk !== this) {
      ...
      if (module.isEntryPoint && !module.chunk.isEmpty) return {
        variable: module.traceExport(name),
        module: module
      };
    ...
    if (name[0] === '*') {
      return {
        variable: undefined,
        module: this.graph.moduleById.get(name.substr(1))
      };
    } // resolve known star exports


    for (var i = 0; i < module.exportAllModules.length; i++) {
      var exportAllModule = module.exportAllModules[i]; // we have to ensure the right export all module
      ...
      for (var i = 0; i < module.dynamicImportResolutions.length; i++) {
        ...
        if (resolution instanceof Module) {
          ...
          if (resolution.chunk === this) {
            ...
            node.setResolution(false, namespace.getName()); // for the module in another chunk, import that other chunk directly
          } else {
            node.setResolution(false);
          } // external dynamic import resolution

        } else if (resolution instanceof ExternalModule) {
          node.setResolution(true); // AST Node -> source replacement
        } else {
          node.setResolution(false);
        }
      }
    ...
    for (var i = 0; i < this.orderedModules.length; i++) {
      ...
      for (var j = 0; j < module.dynamicImportResolutions.length; j++) {
        ...
        if (resolution instanceof Module) {
          ...
          node.renderFinalResolution(code, "\"" + resolution.id + "\""); // AST Node -> source replacement
        } else {
  ...
  Chunk.prototype.finaliseImportMetas = function (options) {
    ...
    for (var i = 0; i < this.orderedModules.length; i++) {
      ...
      for (var _i = 0, _a = module.importMetas; _i < _a.length; _i++) {
        ...
        if (importMeta.renderFinalMechanism(code, this.id, options.format, options.compact)) usesMechanism = true;
      }
    }
  ...
  Chunk.prototype.setIdentifierRenderResolutions = function (options) {
    ...
    var esm = options.format === 'es' || options.format === 'system'; // ensure no conflicts with globals

    Object.keys(this.graph.scope.variables).forEach(function (name) {
      return used[name] = 1;
    });
    ...
    for (var _i = 0, _a = Object.keys(this.exportNames); _i < _a.length; _i++) {
      ...
      if (exportVariable && exportVariable.exportName !== exportName) exportVariable.exportName = exportName;
    }
    ...
    Array.from(this.imports.entries()).forEach(function (_a) {
      ...
      if (module instanceof ExternalModule) {
        if (variable.name === '*') {
          ...
          if (options.interop !== false && (module.exportsNamespace || !esm && module.exportsNames)) {
        ...
        if (chunk.exportMode === 'default') safeName = chunk.name;else safeName = chunk.name + "." + module.chunk.getVariableExportName(variable);
      }
    ...
    this.graph.scope.deshadow(toDeshadow, this.orderedModules.map(function (module) {
      return module.scope;
    }));
  };
  ...
  Chunk.prototype.getChunkDependencyDeclarations = function (options, inputBase) {
    ...
    for (var _i = 0, _a = Object.keys(this.exportNames); _i < _a.length; _i++) {
      ...
      if (exportName[0] === '*') {
        ...
        var module = this.exports.get(variable); // skip local exports
  ...
  Chunk.prototype.getChunkExportDeclarations = function () {
    ...
    var _loop_2 = function (exportName) {
      if (exportName[0] === '*') return "continue";
      var variable = this_2.exportNames[exportName];
      var module = this_2.exports.get(variable); // skip external exports

      if (module.chunk !== this_2) return "continue"; // determine if a hoisted export (function)
      ...
      if (variable instanceof LocalVariable) {
        ...
        variable.declarations.forEach(function (decl) {
          if (decl.type === ExportDefaultDeclaration) {
            if (decl.declaration.type === FunctionDeclaration) hoisted = true;
  ...
  Chunk.prototype.postVisitChunkDependencies = function (visitor) {
    var seen = new Set(); // add in hashes of all dependent chunks and resolved external ids
  ...
  Chunk.prototype.computeFullHash = function (addons, options) {
    var hash = _256(); // own rendered source, except for finalizer wrapping


    hash.update(this.getRenderedHash()); // hash of addons

    hash.update(addons.hash); // output options

    hash.update(options.format); // import names of dependency sources

    hash.update(this.dependencies.length); // add in hashes of all dependent chunks and resolved external ids
  ...
  Chunk.prototype.preRender = function (options, inputBase) {
    ...
    for (var i = 0; i < this.orderedModules.length; i++) {
      ...
      var _d = module.getRenderedExports(),
          renderedExports = _d.renderedExports,
          removedExports = _d.removedExports;
  ...
  Chunk.prototype.merge = function (chunk, chunkList, options, inputBase) {
    ...
    if (this.isEntryModuleFacade || chunk.isEntryModuleFacade) throw new Error('Internal error: Code splitting chunk merges not supported for facades');
    ...
    var thisOldExportNames = this.exportNames; // regenerate internal names
  ...
  Chunk.prototype.generateId = function (pattern, patternName, addons, options, existingNames) {
    ...
    var outName = makeUnique(renderNamePattern(pattern, patternName, function (type) {
      switch (type) {
        ...
        case 'name':
          if (_this.entryModule && _this.entryModule.chunkAlias) return _this.entryModule.chunkAlias;
  ...
  Chunk.prototype.render = function (options, addons, outputChunk) {
    ...
    if (!this.renderedSource) throw new Error('Internal error: Chunk render called before preRender');
    ...
    var hasExports = this.renderedDeclarations.exports.length !== 0 || this.renderedDeclarations.dependencies.some(function (dep) {
      return dep.reexports && dep.reexports.length !== 0;
    });
    var usesTopLevelAwait = this.orderedModules.some(function (module) {
      return module.usesTopLevelAwait;
    });
    ...
    return renderChunk({
      ...
      if (options.sourcemap) {
        ...
        var file = void 0;
        if (options.file) file = resolve(options.sourcemapFile || options.file);else if (options.dir) file = resolve(options.dir, _this.id);else file = resolve(_this.id);
...
function optimizeChunks(chunks, options, CHUNK_GROUPING_SIZE, inputBase) {
  var _loop_1 = function (chunkIndex) {
    ...
    var _loop_2 = function () {
      ...
      chunk.postVisitChunkDependencies(function (dep) {
        return chunkDependencies.add(dep);
      });
...
Parser.prototype.loadPlugins = function loadPlugins(pluginConfigs) {
  ...
  for (var name in pluginConfigs) {
    ...
    plugin(this$1$1, pluginConfigs[name]);
  }
};
...
pp$1.parseTopLevel = function (node) {
  ...
  while (this.type !== types.eof) {
    var stmt = this$1$1.parseStatement(true, true, exports);
    node.body.push(stmt);
  }
...
pp$1.parseSwitchStatement = function (node) {
  ...
  for (var sawDefault = false; this.type !== types.braceR;) {
    if (this$1$1.type === types._case || this$1$1.type === types._default) {
      ...
      cur.consequent.push(this$1$1.parseStatement(true));
    }
  }
...
pp$1.parseBlock = function (createNewLexicalScope) {
  ...
  while (!this.eat(types.braceR)) {
    var stmt = this$1$1.parseStatement(true);
    node.body.push(stmt);
  }
...
pp$1.parseClass = function (node, isStatement) {
  ...
  while (!this.eat(types.braceR)) {
    var member = this$1$1.parseClassMember(classBody);

    if (member && member.type === "MethodDefinition" && member.kind === "constructor") {
      if (hadConstructor) {
        this$1$1.raise(member.start, "Duplicate constructor in the same class");
      }
...
pp$1.parseImportSpecifiers = function () {
  ...
  while (!this.eat(types.braceR)) {
    ...
    this$1$1.checkLVal(node$2.local, "let");
...
pp$3.checkParams = function (node, allowDuplicates) {
  ...
  for (var i = 0, list = node.params; i < list.length; i += 1) {
    var param = list[i];
    this$1$1.checkLVal(param, "var", allowDuplicates ? null : nameHash);
  }
...
var version = "5.7.1"; // The main exported interface (under `self.acorn` when in the
...
var inject = createCommonjsModule(function (module, exports) {
  ...
  function injectDynamicImport(acorn) {
    var tt = acorn.tokTypes; // NOTE: This allows `yield import()` to parse correctly.
...
var UndefinedVariable = function (_super) {
...
var getNewTrackedPaths = function () {
  return {
    tracked: false,
    unknownPath: null,
...
var EntityPathTracker = function () {
...
function createAssetPluginHooks(assetsById, outputBundle, assetFileNames) {
  return {
    emitAsset: function (name, source) {
      if (typeof name !== 'string' || !isPlainName(name)) error({
...
function createTransformEmitAsset(assetsById, emitAsset) {
  ...
  return {
    ...
    emitAsset: function (name, source) {
      ...
      var asset = assetsById.get(assetId); // distinguish transform assets
...
function Uint8ArrayToHexString(buffer) {
  var str = ''; // hex conversion - 2 chars per 8 bit component

  for (var i = 0; i < buffer.length; i++) {
    var num = buffer[i]; // big endian conversion, but whatever
...
function Uint8ArrayXor(to, from) {
  for (var i = 0; i < to.length; i++) to[i] = to[i] ^ from[i];
...
function randomUint8Array(len) {
  ...
  for (var i = 0; i < buffer.length; i++) buffer[i] = Math.random() * (2 << 8);
...
function getRollupDefaultPlugin(options) {
  return {
    ...
    resolveDynamicImport: function (specifier, parentId) {
      if (typeof specifier === 'string') return this.resolveId(specifier, parentId);
    }
  };
}
...
function createPluginDriver(graph, options, pluginCache, watcher) {
  ...
  var _a = createAssetPluginHooks(graph.assetsById),
      emitAsset = _a.emitAsset,
      getAssetFileName$$1 = _a.getAssetFileName,
      setAssetSource = _a.setAssetSource;
  ...
  var pluginContexts = plugins.map(function (plugin) {
    ...
    if (!hasLoadersOrTransforms && (plugin.load || plugin.transform || plugin.transformBundle || plugin.transformChunk)) hasLoadersOrTransforms = true;
    var cacheInstance;
  ...
  function runHookSync(hookName, args, pidx, permitValues, hookContext) {
    ...
    if (hookContext) {
      ...
      if (!context || context === pluginContexts[pidx]) throw new Error('Internal Rollup error: hookContext must return a new context object.');
    }
    ...
    try {
      ...
      if (typeof hook !== 'function') {
        ...
        error({
          ...
          message: "Error running plugin hook " + hookName + " for " + (plugin.name || "Plugin at pos " + (pidx + 1)) + ", expected a function hook."
        });
      }
  ...
  var pluginDriver = {
    ...
    hookSeqSync: function (name, args, hookContext) {
      for (var i = 0; i < plugins.length; i++) runHookSync(name, args, i, false, hookContext);
    },
...
var uncacheablePlugin = function (pluginName) {
  return {
    has: function () {
...
function transform(graph, source, module) {
  ...
  return graph.pluginDriver.hookReduceArg0('transform', [curSource, id], transformReducer, function (pluginContext, plugin) {
    var _a;

    if (plugin.cacheKey) customTransformCache = true;else trackedPluginCache = trackPluginCache(pluginContext.cache);
    var emitAsset;
    _a = createTransformEmitAsset(graph.assetsById, baseEmitAsset), assets = _a.assets, emitAsset = _a.emitAsset;
    return __assign({}, pluginContext, {
...
var Graph = function () {
  function Graph(options, watcher) {
    ...
    this.modules = [];
    this.watchFiles = Object.create(null); // track graph build status as each graph instance is used only once
    ...
    if (options.cache) {
      if (options.cache.modules) for (var _i = 0, _a = options.cache.modules; _i < _a.length; _i++) {
        var module = _a[_i];
    ...
    if (watcher) watcher.on('change', function (id) {
      return _this.pluginDriver.hookSeqSync('watchChange', [id]);
    });
    ...
    acornPluginsToInject.push.apply(acornPluginsToInject, Array.isArray(acornInjectPlugins) ? acornInjectPlugins : acornInjectPlugins ? [acornInjectPlugins] : []);
    this.acornParse = acornPluginsToInject.reduce(function (acc, plugin) {
      return plugin(acc);
    }, acorn).parse;
  }
  ...
  Graph.prototype.loadModule = function (entryName) {
    ...
    return this.pluginDriver.hookFirst('resolveId', [entryName, undefined]).then(function (id) {
      if (id === false) {
        error({
  ...
  Graph.prototype.includeMarked = function (modules) {
    if (this.treeshake) {
      var needsTreeshakingPass = void 0,
          treeshakingPass = 1;
  ...
  Graph.prototype.loadEntryModules = function (entryModules, manualChunks) {
    ...
    if (Array.isArray(entryModules)) {
      ...
      entryModuleIds = entryModuleAliases.map(function (name) {
        return entryModules[name];
      });
    }
    ...
    return Promise.all(entryAndManualChunkIds.map(function (id) {
      return _this.loadModule(id);
    })).then(function (entryAndChunkModules) {
      if (removeAliasExtensions) {
        for (var i = 0; i < entryModuleAliases.length; i++) entryModuleAliases[i] = getAliasName(entryAndChunkModules[i].id, entryModuleAliases[i]);
      }
      ...
      return {
        ...
        entryModuleAliases: entryModuleAliases,
        manualChunkModules: manualChunkModules
      };
    });
  };
  ...
  Graph.prototype.build = function (entryModules, manualChunks, inlineDynamicImports, preserveModules) {
    ...
    return this.loadEntryModules(entryModules, manualChunks).then(function (_a) {
      var entryModules = _a.entryModules,
          entryModuleAliases = _a.entryModuleAliases,
          manualChunkModules = _a.manualChunkModules;
      timeEnd('parse modules', 2); // Phase 2 - linking. We populate the module dependency links and
      ...
      var _b = _this.analyseExecution(entryModules, !preserveModules && !inlineDynamicImports, inlineDynamicImports, manualChunkModules),
          orderedModules = _b.orderedModules,
          dynamicImports = _b.dynamicImports,
          dynamicImportAliases = _b.dynamicImportAliases;
      ...
      if (inlineDynamicImports) {
        ...
        if (entryModules.length > 1) throw new Error('Internal Error: can only inline dynamic imports for single-file builds.');
        ...
        for (var i = 0; i < dynamicImports.length; i++) {
          ...
          if (entryModules.indexOf(dynamicImportModule) === -1) {
            ...
            if (!dynamicImportModule.chunkAlias) dynamicImportModule.chunkAlias = dynamicImportAliases[i];
          }
        }
      }
      ...
      _this.includeMarked(orderedModules); // check for unused external imports
      ...
      timeStart('generate chunks', 2); // TODO: there is one special edge case unhandled here and that is that any module
      ...
      if (!preserveModules) {
        ...
        for (var entryHashSum in chunkModules) {
          ...
          var chunkModulesOrdered = chunkModuleList.sort(function (moduleA, moduleB) {
            return moduleA.execIndex > moduleB.execIndex ? 1 : -1;
          });
      ...
      chunkList = chunkList.filter(function (chunk) {
        return !chunk.isEmpty || chunk.entryModule || chunk.isManualChunk;
  ...
  Graph.prototype.analyseExecution = function (entryModules, graphColouring, inlineDynamicImports, chunkModules) {
    ...
    var visit = function (module) {
      ...
      for (var _b = 0, _c = module.dynamicImportResolutions; _b < _c.length; _b++) {
        ...
        if (!(dynamicModule.resolution instanceof Module)) continue; // If the parent module of a dynamic import is to a child module whose graph
        ...
        if (!graphColouring || !dynamicModule.resolution.chunkAlias && !Uint8ArrayEqual(dynamicModule.resolution.entryPointsHash, curEntry.entryPointsHash)) {
    ...
    return {
      ...
      dynamicImportAliases: dynamicImportAliases
    };
  };
  ...
  Graph.prototype.fetchModule = function (id, importer) {
    ...
    return Promise.resolve(this.pluginDriver.hookFirst('load', [id])).catch(function (err) {
      ...
      if (source && typeof source === 'object' && typeof source.code === 'string') return source; // TODO report which plugin failed
  ...
  Graph.prototype.fetchAllDependencies = function (module) {
    var _this = this; // resolve and fetch dynamic imports where possible
    ...
    return Promise.all(module.sources.map(function (source) {
      return Promise.resolve().then(function () {
        ...
        return _this.pluginDriver.hookFirst('resolveId', [source, module.id]);
        ...
        if (isExternal) {
          ...
          if (!_this.moduleById.has(externalId)) {
            var module_1 = new ExternalModule({
              graph: _this,
              id: externalId
            });
  ...
  function deprecateInputOptions() {
    ...
    if (options.targets) {
      ...
      options.output = options.targets.map(function (target) {
        return __assign({}, target, options.output);
      });
...
var createGetOption = function (config, command) {
...
var getOnWarn = function (config, command, defaultOnWarnHandler) {
  if (defaultOnWarnHandler === void 0) {
    defaultOnWarnHandler = defaultOnWarn;
  }

  return command.silent ? function () {} : config.onwarn ? function (warning) {
...
function mergeOptions(_a) {
  var _b = _a.config,
      config = _b === void 0 ? {} : _b,
      _c = _a.command,
      rawCommandOptions = _c === void 0 ? {} : _c,
      deprecateConfig = _a.deprecateConfig,
      defaultOnWarnHandler = _a.defaultOnWarnHandler;
...
function addUnknownOptionErrors(errors, options, validOptions, optionType, ignoredKeys) {
  ...
  if (unknownOptions.length > 0) errors.push("Unknown " + optionType + ": " + unknownOptions.join(', ') + ". Allowed options: " + validOptions.sort().join(', '));
}
...
function getCommandOptions(rawCommandOptions) {
  ...
  if (rawCommandOptions.globals) {
    ...
    rawCommandOptions.globals.split(',').forEach(function (str) {
      var names = str.split(':');
      command.globals[names[0]] = names[1]; // Add missing Module IDs to external.
...
function getInputOptions$1(rawInputOptions) {
  ...
  if (inputOptions.inlineDynamicImports) {
    ...
    if (inputOptions.inlineDynamicImports) error({
      code: 'INVALID_OPTION',
...
function rollup(rawInputOptions) {
  try {
    ...
    timeStart('BUILD', 1);
    return graph_1.pluginDriver.hookParallel('buildStart').then(function () {
      ...
      timeEnd('BUILD', 1); // TODO: deprecate legacy single chunk return

      var singleChunk;
      var singleInput = typeof inputOptions_1.input === 'string' || inputOptions_1.input instanceof Array && inputOptions_1.input.length === 1; //let imports: string[], exports: string[];
      ...
      function generate(rawOutputOptions, isWrite) {
        ...
        if (inputOptions_1.experimentalCodeSplitting) {
          if (typeof outputOptions.file === 'string' && typeof outputOptions.dir === 'string') error({
            code: 'INVALID_OPTION',
            message: 'Build must set either output.file for a single-file build or output.dir when generating multiple chunks.'
          });

          if (chunks.length > 1) {
            if (outputOptions.format === 'umd' || outputOptions.format === 'iife') error({
              code: 'INVALID_OPTION',
              message: 'UMD and IIFE output formats are not supported with the experimentalCodeSplitting option.'
            });
            if (outputOptions.sourcemapFile) error({
              code: 'INVALID_OPTION',
              message: '"sourcemapFile" is only supported for single-file builds.'
            });
          }

          if (!singleChunk && typeof outputOptions.file === 'string') error({
            code: 'INVALID_OPTION',
            message: singleInput ? 'When building a bundle using dynamic imports, the output.dir option must be used, not output.file. Alternatively set inlineDynamicImports: true to output a single file.' : 'When building multiple entry point inputs, the output.dir option must be used, not output.file.'
          });
        }

        if (!outputOptions.file && inputOptions_1.experimentalCodeSplitting) singleChunk = undefined;
        timeStart('GENERATE', 1); // populate asset files into output
        ...
        var inputBase = commondir(chunks.filter(function (chunk) {
          return chunk.entryModule;
        }).map(function (chunk) {
          return chunk.entryModule.id;
        }));
        return createAddons(graph_1, outputOptions).then(function (addons) {
          // pre-render all chunks
          for (var _i = 0, chunks_2 = chunks; _i < chunks_2.length; _i++) {
            var chunk = chunks_2[_i];
            if (!inputOptions_1.experimentalPreserveModules) chunk.generateInternalExports(outputOptions);
          ...
          for (var i = 0; i < chunks.length; i++) {
            ...
            if (chunk === singleChunk) {
              singleChunk.id = basename(outputOptions.file || (inputOptions_1.input instanceof Array ? inputOptions_1.input[0] : inputOptions_1.input));
          ...
          return Promise.all(chunks.map(function (chunk) {
            ...
            return chunk.render(outputOptions, addons, outputChunk).then(function (rendered) {
              ...
              return graph_1.pluginDriver.hookParallel('ongenerate', [__assign({
                bundle: outputChunk
          ...
          return graph_1.pluginDriver.hookSeq('generateBundle', [outputOptions, outputBundle, isWrite], function (context) {
            return __assign({}, context, generateAssetPluginHooks);
          }).then(function () {
            // throw errors for assets not finalised with a source
            assets.forEach(function (asset) {
              if (asset.fileName === undefined) finaliseAsset(asset, outputBundle, assetFileNames);
            });
          });
        }).then(function () {
          timeEnd('GENERATE', 1);
      ...
      var result = {
        ...
        write: function (outputOptions) {
          if (inputOptions_1.experimentalCodeSplitting && (!outputOptions || !outputOptions.dir && !outputOptions.file)) {
            error({
              code: 'MISSING_OPTION',
              message: 'You must specify output.file or output.dir for the build.'
            });
          } else if (!inputOptions_1.experimentalCodeSplitting && (!outputOptions || !outputOptions.file)) {
            error({
              code: 'MISSING_OPTION',
      ...
      if (inputOptions_1.perf === true) result.getTimings = getTimings;
      return result;
    });
  } catch (err) {
...
function writeOutputFile(graph, build, outputFileName, outputFile, outputOptions) {
  ...
  return writeFile$1(filename, source).then(function () {
    return writeSourceMapPromise;
  }).then(function () {
    return isOutputChunk(outputFile) && graph.pluginDriver.hookSeq('onwrite', [__assign({
      bundle: build
...
var isGlob$1 = function isGlob(str) {
  return typeof str === 'string' && (/[*!?{}(|)[\]]/.test(str) || isExtglob$1(str));
};
/*!
 * is-extglob <https://github.com/jonschlinkert/is-extglob>
 *
...
var isGlob$2 = function isGlob(str) {
  return typeof str === 'string' && (/[*!?{}(|)[\]]/.test(str) || isExtglob$2(str));
};
...
var isGlob$3 = function isGlob(str) {
  return typeof str === 'string' && (/[*!?{}(|)[\]]/.test(str) || isExtglob$3(str));
};
...
function createFilter(include, exclude) {
  const getMatcher = id => isRegexp$1(id) ? id : {
    test: micromatch_1.matcher(resolve(id))
  };
...
var FileWatcher = function () {
  function FileWatcher(id, chokidarOptions, group) {
    ...
    var handleWatchEvent = function (event) {
      if (event === 'rename' || event === 'unlink') {
        ...
        if (+stats.mtime - modifiedTime > 15) _this.trigger(id);
      }
    };
  ...
  FileWatcher.prototype.addTask = function (task, isTransformDependency) {
    if (isTransformDependency === void 0) {
...
var Watcher = function (_super) {
  ...
  function Watcher(configs) {
    ...
    _this.tasks = configs.map(function (config) {
...
var Task = function () {
  function Task(watcher, config) {
    ...
    if (chokidarOptions) {
      chokidarOptions = __assign({}, chokidarOptions === true ? {} : chokidarOptions, {
        ignoreInitial: true
      });
    }
  ...
  Task.prototype.invalidate = function (id, isTransformDependency) {
    ...
    if (isTransformDependency) {
      this.cache.modules.forEach(function (module) {
        if (!module.transformDependencies || module.transformDependencies.indexOf(id) === -1) return; // effective invalidation
  ...
  Task.prototype.run = function () {
    ...
    var options = __assign({}, this.inputOptions, {
      cache: this.cache
    });
    ...
    return rollup(options).then(function (result) {
...
export { version$1 as VERSION, rollup, watch$1 as watch };