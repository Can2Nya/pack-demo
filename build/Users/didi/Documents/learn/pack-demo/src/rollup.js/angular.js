...
(function (window, document, undefined$1) {
  ...
  */
    ...
    if (t1 == t2) {
      if (t1 == 'object') {
        if (isArray(o1)) {
          ...
          for (key in o2) {
            if (!keySet.hasOwnProperty(key) && key.charAt(0) !== '$' && o2[key] !== undefined$1 && !isFunction(o2[key])) return false;
          }
  ...
  function toJson(obj, pretty) {
    if (typeof obj === 'undefined') return undefined$1;
  ...
  function bindJQuery() {
    ...
    jQuery = window.jQuery; // reset to jQuery or default to us.
  ...
  */
  ...
  function JQLite(element) {
    ...
    if (isString(element)) {
      var div = document.createElement('div'); // Read about the NoScope elements here:
  ...
  function jqLiteRemoveData(element, name) {
    ...
    if (expandoStore) {
      ...
      element[jqName] = undefined$1; // ie does not allow deletion of attributes on elements.
    }
  }
  ...
  function jqLiteInheritedData(element, name, value) {
    element = jqLite(element); // if element is the document object work with the html element instead
    ...
    while (element.length) {
      for (var i = 0, ii = names.length; i < ii; i++) {
        if ((value = element.data(names[i])) !== undefined$1) return value;
      }
  ...
  forEach({
    ...
    attr: function (element, name, value) {
      ...
      if (BOOLEAN_ATTR[lowercasedName]) {
        if (isDefined(value)) {
          ...
          return element[name] || (element.attributes.getNamedItem(name) || noop).specified ? lowercasedName : undefined$1;
        }
    ...
    JQLite.prototype[name] = function (arg1, arg2) {
      var i, key; // jqLiteHasClass has only two arguments, but is a getter-only fn, so we need to special-case it
      // in a way that survives minification.
      // jqLiteEmpty takes no arguments but is a setter.

      if (fn !== jqLiteEmpty && (fn.length == 2 && fn !== jqLiteHasClass && fn !== jqLiteController ? arg1 : arg2) === undefined$1) {
        if (isObject(arg1)) {
          ...
          var jj = value === undefined$1 ? Math.min(this.length, 1) : this.length;
      ...
      forEach(type.split(' '), function (type) {
        ...
        if (!eventFns) {
          if (type == 'mouseenter' || type == 'mouseleave') {
            ...
            events[type] = []; // Refer to jQuery's implementation of mouseenter & mouseleave
            ...
            onFn(element, eventmap[type], function (event) {
              var target = this,
                  related = event.relatedTarget; // For mousenter/leave call the handler if related is outside the target.
    ...
    function createInternalInjector(cache, factory) {
      ...
      function instantiate(Type, locals) {
        var Constructor = function () {},
            instance,
            returnedValue; // Check if Type is annotated and use just the given function at n-1 as parameter
    ...
    this.$get = ['$window', '$location', '$rootScope', function ($window, $location, $rootScope) {
      ...
      function scroll() {
        var hash = $location.hash(),
            elm; // empty hash, scroll to the top of the page

        if (!hash) $window.scrollTo(0, 0); // element with given id
        else if (elm = document.getElementById(hash)) elm.scrollIntoView(); // first anchor with given name :-D
        else if (elm = getFirstAnchor(document.getElementsByName(hash))) elm.scrollIntoView(); // no element and hash == 'top', scroll to the top of the page
  ...
  function Browser(window, document, $log, $sniffer) {
    ...
    self.url = function (url, replace) {
      ...
      if (location !== window.location) location = window.location; // setter
      ...
      if (url) {
        ...
        if ($sniffer.history) {
          if (replace) history.replaceState(null, '', url);else {
            history.pushState(null, '', url); // Crazy Opera Bug: http://my.opera.com/community/forums/topic.dml?id=1185462
    ...
    self.onUrlChange = function (callback) {
      if (!urlChangeInit) {
        ...
        if ($sniffer.history) jqLite(window).on('popstate', fireUrlChange); // hashchange event

        if ($sniffer.hashchange) jqLite(window).on('hashchange', fireUrlChange); // polling
    ...
    self.cookies = function (name, value) {
      ...
      if (name) {
        if (value === undefined$1) {
          ...
          if (isString(value)) {
            cookieLength = (rawDocument.cookie = escape(name) + '=' + escape(value) + ';path=' + cookiePath).length + 1; // per http://www.ietf.org/rfc/rfc2109.txt browser must allow at minimum:
            // - 300 cookies
        ...
        if (rawDocument.cookie !== lastCookieString) {
          ...
          for (i = 0; i < cookieArray.length; i++) {
            ...
            if (index > 0) {
              ...
              name = unescape(cookie.substring(0, index)); // the first value that is seen for a cookie is the most
              // specific one.  values for the same cookie name that
              // follow are for less specific paths.

              if (lastCookies[name] === undefined$1) {
  ...
  function $CompileProvider($provide, $$sanitizeUriProvider) {
    var hasDirectives = {},
        ...
        CLASS_DIRECTIVE_REGEXP = /(([\d\w\-_]+)(?:\:([^;]+))?;?)/; // Ref: http://developers.whatwg.org/webappapis.html#event-handler-idl-attributes
    ...
    this.$get = ['$injector', '$interpolate', '$exceptionHandler', '$http', '$templateCache', '$parse', '$controller', '$rootScope', '$document', '$sce', '$animate', '$$sanitizeUri', function ($injector, $interpolate, $exceptionHandler, $http, $templateCache, $parse, $controller, $rootScope, $document, $sce, $animate, $$sanitizeUri) {
      ...
      Attributes.prototype = {
        ...
        $set: function (key, value, writeAttr, attrName) {
          ...
          nodeName = nodeName_(this.$$element); // sanitize a[href] and img[src] values
      ...
      function compile($compileNodes, transcludeFn, maxPriority, ignoreDirective, previousCompileContext) {
        ...
        return function publicLinkFn(scope, cloneConnectFn, transcludeControllers) {
          assertArg(scope, 'scope'); // important!!: we must call our jqLite.clone() since the jQuery one is trying to be smart
      ...
      function compileNodes(nodeList, transcludeFn, $rootElement, maxPriority, ignoreDirective, previousCompileContext) {
        ...
        for (var i = 0; i < nodeList.length; i++) {
          ...
          directives = collectDirectives(nodeList[i], [], attrs, i === 0 ? maxPriority : undefined$1, ignoreDirective);
          ...
          linkFnFound = linkFnFound || nodeLinkFn || childLinkFn; //use the previous context only for the first element in the virtual group
        ...
        function compositeLinkFn(scope, nodeList, $rootElement, boundTranscludeFn) {
          var nodeLinkFn, childLinkFn, node, $node, childScope, childTranscludeFn, i, ii, n; // copy nodeList so that linking doesn't break due to live list updates.
          ...
          for (i = 0, n = 0, ii = linkFns.length; i < ii; n++) {
            ...
            if (nodeLinkFn) {
              ...
              childLinkFn(scope, node.childNodes, undefined$1, boundTranscludeFn);
            }
          }
        }
      }
      ...
      function collectDirectives(node, directives, attrs, maxPriority, ignoreDirective) {
        ...
        switch (nodeType) {
          case 1:
            ...
            addDirective(directives, directiveNormalize(nodeName_(node).toLowerCase()), 'E', maxPriority, ignoreDirective); // iterate over the attributes
            ...
            for (var attr, name, nName, ngAttrName, value, nAttrs = node.attributes, j = 0, jj = nAttrs && nAttrs.length; j < jj; j++) {
              ...
              if (!msie || msie >= 8 || attr.specified) {
                name = attr.name; // support ngAttr attribute binding
      ...
      function applyDirectivesToNode(directives, compileNode, templateAttrs, transcludeFn, jqCollection, originalReplaceDirective, preLinkFns, postLinkFns, previousCompileContext) {
        ...
        for (var i = 0, ii = directives.length; i < ii; i++) {
          ...
          if (directiveValue = directive.scope) {
            newScopeDirective = newScopeDirective || directive; // skip the check for directives with async templates, we'll check the derived sync
        ...
        nodeLinkFn.transclude = hasTranscludeDirective && childTranscludeFn; // might be normal or delayed nodeLinkFn depending on if templateUrl is present
        ...
        function nodeLinkFn(childLinkFn, scope, linkNode, $rootElement, boundTranscludeFn) {
          ...
          if (controllerDirectives) {
            forEach(controllerDirectives, function (directive) {
              ...
              controllerInstance = $controller(controller, locals); // For directives with element transclusion the element is a comment,
          ...
          childLinkFn && childLinkFn(scopeToChild, linkNode.childNodes, undefined$1, boundTranscludeFn); // POSTLINKING
          ...
          function controllersBoundTransclude(scope, cloneAttachFn) {
            var transcludeControllers; // no scope passed
        ...
        if (hasDirectives.hasOwnProperty(name)) {
          for (var directive, directives = $injector.get(name + Suffix), i = 0, ii = directives.length; i < ii; i++) {
            try {
              ...
              if ((maxPriority === undefined$1 || maxPriority > directive.priority) && directive.restrict.indexOf(location) != -1) {
                if (startAttrName) {
      ...
      function getTrustedContext(node, attrNormalizedName) {
        ...
        var tag = nodeName_(node); // maction[xlink:href] can source SVG.  It's not limited to <maction>.
      ...
      function addAttrInterpolateDirective(node, directives, value, name) {
        var interpolateFn = $interpolate(value, true); // no interpolation found -> ignore
          ...
          compile: function () {
            return {
              pre: function attrInterpolatePreLinkFn(scope, element, attr) {
                ...
                interpolateFn = $interpolate(attr[name], true, getTrustedContext(node, name)); // if attribute was updated so that there is no interpolation going on we don't want to
                // register any observers

                if (!interpolateFn) return; // TODO(i): this should likely be attr.$set(name, iterpolateFn(scope) so that we reset the
                // actual attr value
  ...
  function headersGetter(headers) {
    var headersObj = isObject(headers) ? headers : undefined$1;
  ...
  function $HttpProvider() {
    ...
    this.$get = ['$httpBackend', '$browser', '$cacheFactory', '$rootScope', '$q', '$injector', function ($httpBackend, $browser, $cacheFactory, $rootScope, $q, $injector) {
      ...
      function $http(requestConfig) {
        ...
        var xsrfValue = urlIsSameOrigin(config.url) ? $browser.cookies()[config.xsrfCookieName || defaults.xsrfCookieName] : undefined$1;
        ...
        var serverRequest = function (config) {
          ...
          var reqData = transformData(config.data, headersGetter(headers), config.transformRequest); // strip content-type if data is undefined
        ...
        var chain = [serverRequest, undefined$1];
        ...
        function mergeHeaders(config) {
          var defHeaders = defaults.headers,
              ...
              reqHeaderName;
          defHeaders = extend({}, defHeaders.common, defHeaders[lowercase(config.method)]); // execute if header value is function

          execHeaders(defHeaders);
          execHeaders(reqHeaders); // using for-in instead of forEach to avoid unecessary iteration after header has been found
  ...
  function createHttpBackend($browser, XHR, $browserDefer, callbacks, rawDocument) {
    var ABORTED = -1; // TODO(vojta): fix the signature
    ...
    return function (method, url, post, callback, headers, timeout, withCredentials, responseType) {
      ...
      function completeRequest(callback, status, response, headersString) {
        var protocol = urlResolve(url).protocol; // cancel timeout and subsequent timeout promise resolution

        timeoutId && $browserDefer.cancel(timeoutId);
        jsonpDone = xhr = null; // fix status code for file protocol (it's always 0)

        status = protocol == 'file' && status === 0 ? response ? 200 : 404 : status; // normalize IE bug (http://bugs.jquery.com/ticket/1450)
  ...
  function LocationHtml5Url(appBase, basePrefix) {
    ...
    this.$$rewrite = function (url) {
      ...
      if ((appUrl = beginsWith(appBase, url)) !== undefined$1) {
        prevAppUrl = appUrl;

        if ((appUrl = beginsWith(basePrefix, appUrl)) !== undefined$1) {
          return appBaseNoFile + (beginsWith('/', appUrl) || appUrl);
        } else {
          return appBase + prevAppUrl;
        }
      } else if ((appUrl = beginsWith(appBaseNoFile, url)) !== undefined$1) {
  ...
  var promiseWarning; // Sandboxing Angular Expressions
  // ------------------------------
  ...
  var OPERATORS = {
    ...
    '+': function (self, locals, a, b) {
      ...
      return isDefined(b) ? b : undefined$1;
    },
    '-': function (self, locals, a, b) {
      a = a(self, locals);
      b = b(self, locals);
    ...
    lex: function (text) {
      ...
      while (this.index < this.text.length) {
        ...
        if (this.is('"\'')) {
          ...
          this.readIdent(); // identifiers can only be if the preceding char was a { or ,
  ...
  Parser.prototype = {
    ...
    objectIndex: function (obj) {
      ...
      return extend(function (self, locals) {
        ...
        assign: function (self, value, locals) {
          var key = indexFn(self, locals); // prevent overwriting of Function.constructor which would break ensureSafeObject check
    ...
    functionCall: function (fn, contextGetter) {
      ...
      return function (scope, locals) {
        ...
        ensureSafeObject(fnPtr, parser.text); // IE stupidity! (IE doesn't have apply for some native functions)
  ...
  function cspSafeGetterFn(key0, key1, key2, key3, key4, fullExp, options) {
    ...
    return !options.unwrapPromises ? function cspSafeGetter(scope, locals) {
      ...
      if (pathVal === null || pathVal === undefined$1) return pathVal;
      pathVal = pathVal[key0];
      if (!key1 || pathVal === null || pathVal === undefined$1) return pathVal;
      pathVal = pathVal[key1];
      if (!key2 || pathVal === null || pathVal === undefined$1) return pathVal;
      pathVal = pathVal[key2];
      if (!key3 || pathVal === null || pathVal === undefined$1) return pathVal;
      pathVal = pathVal[key3];
      if (!key4 || pathVal === null || pathVal === undefined$1) return pathVal;
  ...
  function getterFn(path, options, fullExp) {
    ...
    if (options.csp) {
      if (pathKeysLength < 6) {
        ...
        fn = function (scope, locals) {
          ...
          do {
            ...
            locals = undefined$1; // clear after first iteration
      ...
      forEach(pathKeys, function (key, index) {
        ...
        code += 'if(s === null || s === undefined) return s;\n' + 'l=s;\n' + 's=' + (index // we simply dereference 's' on any .dot notation
    ...
    this.$get = ['$injector', '$exceptionHandler', '$parse', '$browser', function ($injector, $exceptionHandler, $parse, $browser) {
      ...
      Scope.prototype = {
        ...
        $new: function (isolate) {
          ...
          if (isolate) {
            child = new Scope();
            child.$root = this.$root; // ensure that there is just one async queue per $rootScope and its children
          ...
          do {
            ...
            traverseScopesLoop: do {
              ...
              if (watchers = current.$$watchers) {
                ...
                while (length--) {
                  try {
                    watch = watchers[length]; // Most common watches are on primitives, in which case we can short
        ...
        $destroy: function () {
          ...
          if (this.$$nextSibling) this.$$nextSibling.$$prevSibling = this.$$prevSibling; // This is bogus code that works around Chrome's GC leak
        ...
        $emit: function (name, args) {
          ...
          do {
            ...
            if (stopPropagation) return event; //traverse upwards
  ...
  function $$SanitizeUriProvider() {
    ...
    this.$get = function () {
      return function sanitizeUri(uri, isImage) {
        ...
        var normalizedVal; // NOTE: urlResolve() doesn't support IE < 8 so we don't sanitize for that case.
  ...
  function $SceDelegateProvider() {
    ...
    this.$get = ['$injector', function ($injector) {
      ...
      function trustAs(type, trustedValue) {
        ...
        if (trustedValue === null || trustedValue === undefined$1 || trustedValue === '') {
          return trustedValue;
      ...
      function getTrusted(type, maybeTrusted) {
        if (maybeTrusted === null || maybeTrusted === undefined$1 || maybeTrusted === '') {
          return maybeTrusted;
        }
      ...
      return {
        ...
        link: function (scope, element, attr) {
          attr.$observe(normalized, function (value) {
            if (!value) return;
            attr.$set(attrName, value); // on IE, if "ng:src" directive declaration is used and "src" attribute doesn't exist
  ...
  function FormController(element, attrs) {
    ...
    form.$valid = true;
    form.$invalid = false;
    parentForm.$addControl(form); // Setup initial state of the control

    element.addClass(PRISTINE_CLASS);
    toggleValidCss(true); // convenience method for easy toggling of classes
            ...
            pre: function (scope, formElement, attr, controller) {
              ...
              if (parentFormCtrl) {
                formElement.on('$destroy', function () {
                  ...
                  if (alias) {
                    setter(scope, alias, undefined$1, alias);
                  }
  ...
  */
  ...
  function textInputType(scope, element, attr, ctrl, $sniffer, $browser) {
    ...
    var listener = function () {
      if (composing) return;
      var value = element.val(); // By default we will trim the value
  ...
  var NgModelController = ['$scope', '$exceptionHandler', '$attrs', '$element', '$parse', function ($scope, $exceptionHandler, $attr, $element, $parse) {
    ...
    this.$setValidity = function (validationErrorKey, isValid) {
      ...
      // jshint -W018
      if ($error[validationErrorKey] === !isValid) return; // jshint +W018
    ...
    this.$setViewValue = function (value) {
      this.$viewValue = value; // change to dirty
    ...
    $scope.$watch(function ngModelWatch() {
      var value = ngModelGet($scope); // if scope model value and ngModel value are out of sync
  ...
  var ngBindDirective = ngDirective(function (scope, element, attr) {
    ...
    scope.$watch(attr.ngBind, function ngBindWatchAction(value) {
      ...
      // jshint -W041
      element.text(value == undefined$1 ? '' : value);
    });
  });
  /**
   * @ngdoc directive
          ...
          if (toBoolean(value)) {
            if (!childScope) {
              ...
              $transclude(childScope, function (clone) {
                clone[clone.length++] = document.createComment(' end ngIf: ' + $attr.ngIf + ' '); // Note: We only need the first/last node of the cloned nodes.
  ...
  var ngRepeatDirective = ['$parse', '$animate', function ($parse, $animate) {
    ...
    return {
      ...
      link: function ($scope, $element, $attr, ctrl, $transclude) {
        ...
        keyIdentifier = match[2]; // Store a list of elements from previous run. This is a hash where key is the item from the
        // iterator, and the value is objects with following properties.
        //   - scope: bound scope
        //   - element: previous element.
        //   - index: position

        var lastBlockMap = {}; //watch props
        ...
        $scope.$watchCollection(rhs, function ngRepeatAction(collection) {
          ...
          arrayLength = collectionKeys.length; // locate existing items
          ...
          for (index = 0, length = collectionKeys.length; index < length; index++) {
            ...
            childScope.$middle = !(childScope.$first || childScope.$last); // jshint bitwise: false

            childScope.$odd = !(childScope.$even = (index & 1) === 0); // jshint bitwise: true
            ...
            if (!block.scope) {
              $transclude(childScope, function (clone) {
                ...
                previousNode = clone;
                block.scope = childScope; // Note: We only need the first/last node of the cloned nodes.
                // However, we need to keep the reference to the jqlite wrapper as it might be changed later
                // by a directive with templateUrl when it's template arrives.
